...
public class LongRegressionAlgoOld extends Algo {
	private Serie ema50;
	private Serie ema20;
	...
	public LongRegressionAlgoOld() {
	}

	public LongRegressionAlgoOld(double keltnerMultiplier, int period) {
	...
	public Algo getInstance() {
		return new LongRegressionAlgoOld(keltnerMultiplier, period);
	}
	...
	protected void processToday(HistoricalData hd, BookOfRecord bor) {
		...
		this.regLong2(hd, bor);
//		this.regLongMax(hd, bor);
	}

	protected void regLong(HistoricalData hd, BookOfRecord bor) {
	 ...
	 * @param bor
	 */
	protected void regLong2(HistoricalData hd, BookOfRecord bor) {
		// check trend is going up
		boolean trendUp = lr.getPercentYearlyRateOfChange() >= 0.1;
	...
	protected void regLongMax(HistoricalData hd, BookOfRecord bor) {
		...
		if (trendDown && hd.midLow.get(0) < upperKC.get(0)
				...
				&& hd.midHigh.get(0) > upperKC.get(0)
				&& hd.midOpen.get(0) < hd.midClose.get(0)) {
